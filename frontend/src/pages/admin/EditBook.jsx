...
const EditBook = () => {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get(`${backendUrl}/api/admin/books/${id}`);
        if (data.success) {
          setBook({
            ...data.book,
 ...
 const handleArrayChange = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (bookCover) {
        ...
        const uploadRes = await axios.post(`${backendUrl}/api/admin/editBook`, formData, {
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto p-6">
      <h2 className="text-2xl sm:text-4xl font-bold text-center text-purple-600">Edit Book</h2>
